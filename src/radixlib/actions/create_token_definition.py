...
class CreateTokenDefinition(Serializable):
    ...
    def __init__(
        ...
        to_account: Optional[AccountIdentifier] = None,
    ) -> None:
        """ Instantiates a new CreateTokenDefinition action used for the creation of new tokens.

        Args:
            owner (AccountIdentifier): An account identifier of the account which will be the owner 
                of the token.
            ...
            to_account (AccountIdentifier, optional): An optional argument which defaults to None, 
                and defines the account to send the fixed supply tokens to.
        ...
        if is_supply_mutable and (token_supply is not None or to_account is not None):
            raise ValueError(
        ...
        elif not is_supply_mutable and (token_supply is None or to_account is None):
            raise ValueError(
                "You must specify 'token_supply' and the 'to_account' arguments when creating a "
                "fixed supply token."
            )

        self.owner: AccountIdentifier = owner
        self.name: str = name
        ...
        self.to_account: Optional[AccountIdentifier]  = to_account
    ...
    def from_dict(
        ...
        return cls(
            owner = AccountIdentifier(
                address = dictionary['token_properties']['owner']['address']
            ),
            ...
            to_account = None if dictionary.get('to_account') is None else AccountIdentifier(
                address = dictionary['to_account']['address']
            )
        )
