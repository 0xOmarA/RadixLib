...
def main() -> None:
    ...
    batch_action_builder: radix.ActionBuilder = wallet.action_builder
    for tx in transactions[::-1]:
        ...
        for rri, amount in tokens_tally.items():
            ...
            batch_action_builder = batch_action_builder.token_transfer(
    ...
    if not batch_action_builder.to_action_list():
        print("Did not find any actions to submit to the ledger.")
        exit()

    tx_hash: str = wallet.build_sign_and_send_transaction(
        actions = batch_action_builder,
    )
